...
func TestPackPacketMessageClasses(t *testing.T) {
	tests := []struct {
		...
		// System realtime carries CIN 0xF, "Single Byte" -- not the 0x5 the
		// tune request above uses, which is single-byte System Common.
		{"clock", []byte{0xF8}, []byte{0x0F, 0xF8, 0x00, 0x00}},
		{"active sensing", []byte{0xFE}, []byte{0x0F, 0xFE, 0x00, 0x00}},
		{"system reset", []byte{0xFF}, []byte{0x0F, 0xFF, 0x00, 0x00}},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got := PackPacket(tc.msg)
...
func TestUnpackPacketsMixedBuffer(t *testing.T) {
	buf := []byte{
		...
		0x05, 0xF8, 0x00, 0x00, // realtime clock from a peer that packs it as CIN 0x5
