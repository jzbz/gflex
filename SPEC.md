...
| 16 | Bootloader re-enumeration details | **It just works, and the jump is confirmable.** A full `firmware flash` ran end to end on unit `58b4f621`, 2026-08-21: the application-mode serial was read, `CMD_JUMP_APP_TO_BOOTLOADER` sent, the device left the bus, and the vendor-class interface opened **within the 8 s retry window** (`ConnectRetryWindow`) with no manual intervention. The bootloader reported the **same serial**, so §10.1's identity check across the excursion holds in practice. 165 pages of 320 bytes streamed in acknowledged mode, the device's CRC came back `0x30` matching the image's declared value, the jump back to the application succeeded, and every setting replayed. No PID change was needed to find it: matching on the vendor ID alone (§4.4) located the device in both modes. The `SET_CONTROL_LINE_STATE` control transfer was never required. |
